...
const VehicleSelection = () => {
  ...
  const [isVehicleAllocated, setIsVehicleAllocated] = useState(false);
  const [allVehiclesAllocated, setAllVehiclesAllocated] = useState(false);
  ...
  useEffect(() => {
    const storedCopsData = JSON.parse(localStorage.getItem("selectedCopsData"));
    setSelectedCopsData(storedCopsData ? storedCopsData : []);
  }, []);
  ...
  const handleVehicleSelect = (cop, city) => {
    ...
    const isAllocated = selectedCopsData.some(
      (data) => data.cop.id === cop.id && data.vehicle
    );
  ...
  useEffect(() => {
    const filteredSelectedCopsData = selectedCopsData.filter(
      (data) => data.vehicle !== null
    );
    const isAllAllocated = filteredSelectedCopsData.length === 3;
    const hasNullValues = selectedCopsData.some(
      (data) => data.vehicle === null
    );
    setAllVehiclesAllocated(isAllAllocated && !hasNullValues);
  }, [selectedCopsData]);

  const handleAllocateVehicle = (vehicle) => {
    const updatedSelectedCopsData = selectedCopsData.map(
      ({ cop, city, vehicle: allocatedVehicle }) => {
    ...
    localStorage.setItem(
      "selectedCopsData",
      JSON.stringify(updatedSelectedCopsData)
    );
  ...
  return (
    <div style={{ margin: "50px 20px 100px 20px" }}>
      <h4>
        Allocate Vehicle to respective cop to find the notorious criminal escape
        artist !!
      </h4>
          ...
          {selectedCopsData.map(({ cop, city, vehicle }) => (
            <div key={cop.id} className="city-main">
              ...
              <button
                ...
                onClick={() => handleVehicleSelect(cop, city)}
              >
              ...
              {vehicles.map((vehicle) => (
                <div className="cities-card" key={vehicle.id}>
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => handleAllocateVehicle(vehicle)}
                    disabled={vehicle.count === 0}
                  >
                    {vehicle.count === 0 ? "Allocated" : "Allocate"}
                  </button>
                  <h6
                    className="cities-card-title"
                    style={{ color: "#282c34" }}
                  >
                    Availability: {vehicle.count} <br /> Range: {vehicle.range}{" "}
                    KM
                  </h6>
                </div>
              ))}
