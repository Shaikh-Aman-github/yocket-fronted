...
const CitySelection = ({ onSelectCity }) => {
  ...
  const handleAllocateCity = (city) => {
    // Use cop's ID as the key in allocatedCities
    setAllocatedCities(prevAllocatedCities => ({
      ...prevAllocatedCities,
      [selectedCop.id]: { cop: selectedCop, city: city },
    }));
  
    // Store allocatedCities in localStorage
    localStorage.setItem('allocatedCities', JSON.stringify({
      ...allocatedCities,
      [selectedCop.id]: { cop: selectedCop, city: city },
    }));
    setShowPopup(false);
  ...
  useEffect(() => {
    const allocatedCitiesData = JSON.parse(localStorage.getItem('allocatedCities'));
  ...
  const handleRemoveCity = (cop) => {
    // Use the provided cop's ID to remove the allocated city
    ...
    // Update localStorage
    localStorage.setItem('allocatedCities', JSON.stringify(updatedAllocatedCities));
  };
  

  const handleAllocateVehicle = () => {
    const selectedCopsData = Object.values(allocatedCities);
    // Store selectedCopsData in local storage
  ...
  return (
    <div style={{ margin: "50px 20px" }}>
      <h4>Allocate city to respective cop to find the notorious criminal escape artist !!</h4>
  ...
  <div className='card-main'>
    {cops.map(cop => {
      ...
      return (
        <div key={cop.id}>
          <div className='card'>
            ...
            <button className="Allocate-City" type="button" onClick={() => handleAllocateCityClick(cop)} disabled={isCityAlreadySelected}>Allocate City</button>
          </div>
          {isCityAlreadySelected && (
            <>
              ...
              <div className='card'>
                ...
                <img className="cop-image" src={allocatedCity.city.image} alt={cop.image} />
                <button className="Allocate-City" onClick={() => handleRemoveCity(cop)}>Remove City</button>
              </div>
            </>
          )}
        </div>
      ...
      {showPopup && (
        <div className="popup-overlay">
          <div className="popup-content">
            ...
            <p>Allocate City to: {selectedCop && selectedCop.name} to find criminal</p>
            <hr />
            <div className='cities-card-main'>
              {cities.map(city => {
                const isCityAlreadyAllocated = Object.values(allocatedCities).some(allocatedCity => allocatedCity.city.id === city.id);
                return (
                  <div className='cities-card' key={city.id}>
                    <h5 className='cities-card-title'>{city.name}</h5>
                    <img className="cities-image" src={city.image} alt={city.name} title={city.dec} />
                    <button className="Select-City-btn" type="button" onClick={() => handleAllocateCity(city)} disabled={isCityAlreadyAllocated}>{isCityAlreadyAllocated ? "Allocated" : "Allocate"}</button>
                  </div>
                );
              })}
            </div>
      ...
      {Object.keys(allocatedCities).length === 3 && (
        <button className="startGame" style={{ marginTop: "20px" }} onClick={handleAllocateVehicle}>Allocate Vehicle</button>
      )}
    </div>
  );
};
